...
# IDENTIFYING POSSIBLE HOMOZYGOUS DELETIONS IN THE REAL DATA SETS FOR NGS2 AND NGS3
# USES AS IDENTIFICATION VARIANTS WHICH FALL OUTSIDE OF TRENDLINE (CALCULATED USING SPIKED DATA) + ONE MAD VARIANCE
# AND THE THOSE WHICH FALL OUTSIDE OF THE TRENDLINE FORMED FROM + ONE MAD VARIANCE
# OUTPUTS ONE FILE, CONTAINING ONLY POSSIBLE VARIANTS WHICH APPEAR IN 2 OR LESS SAMPLES
...
# ----------------------------------------------------------------------------------------------------------------
# CREATE OUTPUT FILES
NGSOutput = open('/data/bioinf/home/cls87/SpikingData/HomDel/HomDel.txt','w')

NGSOutput.write('Mito Ranking	Protein	Protein Description	Gene	Chromosome	Interval Start	Interval End	Average Coverage	Identified Sample	Identified Value	Difference	NGS00004 Coverage	NGS00005 Coverage	NGS00006 Coverage	NGS00007 Coverage	NGS0008 Coverage	NGS00009 Coverage	NGS00011 Coverage	NGS00012 Coverage	NGS00013 Coverage	NGS00014 Coverage	NGS00015 Coverage	NGS00004 Score	NGS00005 Score	NGS00006 Score	NGS00007 Score	NGS00008 Score	NGS00009 Score	NGS00011 Score	NGS00012 Score	NGS00013 Score	NGS00014 Score	NGS00015 Score\n')
...
# ----------------------------------------------------------------------------------------------------------------
# SET UP SOME LISTS FOR DATA STORAGE
NGS2CovList = []
NGS3CovList = []
...
for partNo in range(1,17):
	...
	for d in intervalSummary:
		...
		# CREATE A LIST TO STORE THOSE IDENTIFIED AS POSSIBLE HOMOZYGOUS DELETIONS
		identifiedList = []
		...
		# IF THERE SINGLE SAMPLES IDENTIFIED AS HAVING A POSSIBLE HOMOZYGOUS DELETION
		if 0 < len(identifiedList) <2:
			for sample in identifiedList:
				if sample in NGS2:
					...
					chrm = chromosome[3:]			
					NGSOutput.write(str(mitoRank) +'\t'+ mitoProtein +'\t'+ proteinDesc +'\t'+ strGenes +'\t'+ chrm +'\t'+ str(start) +'\t'+ str(end) +'\t')
					roundDiff = round(intDiff,2)
					NGSOutput.write(str(avgCov) +'\t'+ sampleNGS + '\t'+ str(NGS2intervalList[position]) +'\t'+ str(roundDiff)+'\t')

					for entry in NGS2covList:
						NGSOutput.write(str(entry) + '\t')
				...
				if sample in NGS3:
					...
					NGSOutput.write(str(mitoRank) +'\t'+ mitoProtein +'\t'+ proteinDesc +'\t'+ strGenes +'\t'+ chrm +'\t'+ str(start) +'\t'+ str(end) +'\t')
					roundDiff = round(intDiff,2)
					NGSOutput.write(str(avgCov) +'\t'+ sampleNGS + '\t'+ str(NGS3intervalList[position]) +'\t'+ str(roundDiff)+'\t')
